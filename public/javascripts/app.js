function savePillToLocalHost(pill){
  var medsOnTime = { pills: {} }
  if (localStorage.medsOnTime){
    medsOnTime = JSON.parse(localStorage.medsOnTime);
  }

  medsOnTime.pills[pill.name] = pill;
  localStorage.setItem("medsOnTime", JSON.stringify(medsOnTime))
}

function fetchPillsFromLocalHost(){
  if (localStorage.medsOnTime) return JSON.parse(localStorage.medsOnTime).pills
  else return false
}
